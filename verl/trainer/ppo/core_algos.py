...
def compute_policy_loss(
    ...
    # Covariance-aware baselines and entropy loss (minimal integration)
    if loss_method and loss_method != "":
        mask = (response_mask > 0).float()
        N = mask.sum()
        if N > 0:
            # token-wise centered cross-product
            lp_mean = (log_prob * mask).sum() / (N + 1e-8)
            adv_mean = (advantages * mask).sum() / (N + 1e-8)
            covs = (log_prob - lp_mean) * (advantages - adv_mean)

            if loss_method == "clip_cov":
                # randomly detach high-cov tokens by zeroing their contributions
                cov_lb, cov_ub = 1.0, 5.0
                select_ratio = 0.0002
                cond = (covs > cov_lb) & (covs < cov_ub) & (response_mask > 0)
                idx = torch.nonzero(cond.flatten(), as_tuple=False).flatten()
                if idx.numel() > 0:
                    k = int(max(1, min(idx.numel(), int(select_ratio * idx.numel()))))
                    perm = torch.randperm(idx.numel(), device=idx.device)[:k]
                    pick = idx[perm]
                    keep = torch.ones_like(response_mask, dtype=torch.float).flatten()
                    keep[pick] = 0.0
                    keep = keep.view_as(response_mask)
                    pg_losses = pg_losses * keep

            elif loss_method == "kl_cov":
                # apply local KL penalty on top-k covariance tokens
                select_ratio = 0.0002
                kl_coef = 1.0
                abs_cov = torch.abs(covs) * mask
                flat_cov = abs_cov.flatten()
                k = int(max(1, min(int(select_ratio * int(N.item())), flat_cov.numel())))
                if k > 0:
                    _, topk_idx = torch.topk(flat_cov, k=k, largest=True)
                    sel_mask_flat = torch.zeros_like(flat_cov)
                    sel_mask_flat[topk_idx] = 1.0
                    sel_mask = sel_mask_flat.view_as(response_mask)
                    kl_penalty = torch.abs(log_prob - old_log_prob)
                    pg_losses = pg_losses + kl_coef * kl_penalty * sel_mask

            elif loss_method == "entro_loss":
                # simple entropy-like regularization using -log_prob as proxy
                entro_coef = 0.001
                ent_like = -log_prob
                pg_losses = pg_losses - entro_coef * ent_like
...
def compute_alpha_js_divergence_efficient(log_prob, ref_log_prob, alpha, mask):
    """Alpha-JS散度: alpha→0为反向KL, alpha→1为前向KL"""
    log_1_minus_alpha = torch.log(1 - alpha + 1e-10)
    log_alpha = torch.log(alpha + 1e-10)
    
    log_mix = torch.logsumexp(
        torch.stack([log_1_minus_alpha + ref_log_prob, log_alpha + log_prob], dim=0),
        dim=0
    )
    
    kl_ref_mix = kl_penalty(ref_log_prob, log_mix, kl_penalty="low_var_kl")
    kl_theta_mix = kl_penalty(log_prob, log_mix, kl_penalty="low_var_kl")
    
    return ((1 - alpha) * kl_ref_mix + alpha * kl_theta_mix) * mask
