...
class DataParallelPPOActor(BasePPOActor):
    def __init__(self, config, actor_module: nn.Module, actor_optimizer: torch.optim.Optimizer = None):
        ...
        self.CLIP_RATIO_THRESHOLD = self.config.get("clip_ratio_threshold_tmp", 0.2)
    ...
    def _get_adjusted_clip_ratio_low(self, base_ratio: float) -> float:
        enable_adjustment = getattr(self.config, 'enable_clip_ratio', False)
        # enable_adjustment = getattr(self.config, 'enable_clip_ratio', True)
        adjustment_factor = getattr(self.config, 'low_factor', 1.4)
        
        if enable_adjustment and base_ratio == self.CLIP_RATIO_THRESHOLD:
            return base_ratio * adjustment_factor
        return base_ratio
    def _get_adjusted_clip_ratio_high(self, base_ratio: float) -> float:
        # enable_adjustment = getattr(self.config, 'enable_clip_ratio', False)
        enable_adjustment = getattr(self.config, 'enable_clip_ratio', True)
        adjustment_factor = getattr(self.config, 'high_factor', 1.4)
        if enable_adjustment and base_ratio == self.CLIP_RATIO_THRESHOLD:
            return base_ratio * adjustment_factor
        return base_ratio
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    # token_level_rewards or token_level_scores
                    clip_ratio = self.config.clip_ratio
                    clip_ratio_low = (
                        self._get_adjusted_clip_ratio_low(self.config.clip_ratio_low) if self.config.clip_ratio_low is not None else clip_ratio
                    )
                    clip_ratio_high = (
                        self._get_adjusted_clip_ratio_high(self.config.clip_ratio_high) if self.config.clip_ratio_high is not None else clip_ratio
                    )
                    ...
                    if self.config.get("use_entropy_advantage", False):                         
                        calculate_entropy = True
                    ...
                    loss_method = None
                    if self.config.get("use_entropy_advantage", False):
                        entropy_mode = self.config.get("entropy_advantage_mode", "original")
                        print("=====", entropy_mode)
                        if entropy is not None:
                            ...
                            print("Warning: entropy is None, skipping entropy advantage calculation")
                    ...
                    if self.config.get("use_divg_loss", False):
                        ref_log_prob = data["ref_log_prob"]
                        mask = (response_mask > 0).float()
                        
                        probs = torch.exp(log_prob)
                        sample_advantages = (advantages * mask).sum(dim=-1) / (mask.sum(dim=-1) + 1e-8)
                        sample_probs = (probs * mask).sum(dim=-1) / (mask.sum(dim=-1) + 1e-8)
                        is_learned = (sample_advantages > sample_advantages.median()) & (sample_probs > sample_probs.median())
                        
                        sampling_ratio = self.config.get("divergence_sampling_ratio", 0.1)
                        learned_indices = torch.nonzero(is_learned, as_tuple=False).flatten()
                        if learned_indices.numel() > 0:
                            num_selected = max(1, int(learned_indices.numel() * sampling_ratio))
                            selected = learned_indices[torch.randperm(learned_indices.numel(), device=learned_indices.device)[:num_selected]]
                            final_mask = torch.zeros_like(is_learned)
                            final_mask[selected] = True
                            final_mask = final_mask.unsqueeze(-1).expand_as(mask) * mask
                            
                            divergence_mode = self.config.get("divergence_mode", "js")
                            
                            if divergence_mode == "js":
                                prob_ref = torch.exp(ref_log_prob)
                                prob_theta = probs
                                prob_m = 0.5 * (prob_ref + prob_theta)
                                log_prob_m = torch.log(prob_m + 1e-10)
                                
                                kl_theta_m = kl_penalty(
                                    logprob=log_prob, 
                                    ref_logprob=log_prob_m, 
                                    kl_penalty=self.config.kl_loss_type
                                )
                                
                                kl_ref_m = kl_penalty(
                                    logprob=ref_log_prob, 
                                    ref_logprob=log_prob_m, 
                                    kl_penalty=self.config.kl_loss_type
                                )
                                
                                kld = 0.5 * (kl_theta_m + kl_ref_m)
                            ...
                            divergence_loss = agg_loss(loss_mat=kld, loss_mask=final_mask, loss_agg_mode=loss_agg_mode)
                            divergence_coef = self.config.get("divergence_coef", 0.01)
                            policy_loss = policy_loss + divergence_loss * divergence_coef
                            
                            micro_batch_metrics["actor/divergence_loss"] = divergence_loss.detach().item()
                            micro_batch_metrics["actor/num_selected_samples"] = final_mask.any(dim=-1).sum().item()
                            micro_batch_metrics["actor/total_learned_samples"] = is_learned.sum().item()
