...
class Tracking:
    ...
    supported_backend = ["wandb", "mlflow", "swanlab", "vemlp_wandb", "tensorboard", "console", "clearml", "hope"]
    ...
    def __init__(self, project_name, experiment_name, default_backend: str | list[str] = "console", config=None):
        ...
        if "hope" in default_backend:
            self.logger["hope"] = _HopeAdapter(project_name, experiment_name, config)
    ...
    def __del__(self):
        ...
        if "hope" in self.logger:
            self.logger["hope"].finish()
...
class _HopeAdapter:
    def __init__(self, project_name, experiment_name, config):
        ...
        # this logger will only be initialized on the head node,
        # so we need to set the rank to 0 to avoid the error of hope logger.
        world_size = int(config["trainer"]["n_gpus_per_node"]) * int(config["trainer"]["nnodes"])
        os.environ["HOPE_TRACKING_RANK"] = str(world_size - 1)

        # if "HOPE_TRACKING_RANK" in os.environ:
        #     os.environ.pop("HOPE_TRACKING_RANK")

        if "TENSORBOARD_DIR" in os.environ:
            tensorboard_dir = os.environ.get("TENSORBOARD_DIR", f"tensorboard_log/{project_name}/{experiment_name}")
        else:
            tensorboard_dir = os.path.join(config["trainer"]["default_local_dir"], "tensorboard_log", project_name, experiment_name)
            print("Warning: TENSORBOARD_DIR is not set, using default local dir to save tensorboard log.")

        Path(tensorboard_dir).mkdir(parents=True, exist_ok=True)
        print(f"Saving hope tensorboard log to {tensorboard_dir}.")
        self._tf_writer = SummaryWriter(tensorboard_dir)
        self.writer = tracking.get_scalar_writer(self._tf_writer, dual_write=True)
