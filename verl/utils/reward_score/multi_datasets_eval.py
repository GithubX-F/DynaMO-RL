...
def compute_score(
    ...
    if eval_method == "qwen":
        ...
        return compute_score_fast(solution_str, ground_truth, strict_box_verify, pause_tokens_index)
    else:
...
# 如何修改成只返回一种情况的？ 优先计算compute_score_dapo，dapo_result.get("acc", False)有结果就不用计算qwen的了
...
def compute_score_fast(
    ...
    """Compute the reward score using both DAPO and Qwen methods (fast version).
    优先计算DAPO，如果结果为正，直接返回，不计算Qwen."""
    try:
        # 优先计算DAPO方法
        dapo_result = compute_score_dapo(solution_str, ground_truth, strict_box_verify, pause_tokens_index)
        dapo_correct = dapo_result.get("acc", False)
        ...
        # 只有DAPO为False时才计算Qwen方法
        qwen_result = compute_score_qwen(solution_str, ground_truth)
        qwen_correct = qwen_result.get("acc", False)
        
        # 返回结果
        return {
            "score": 1.0 if qwen_correct else -1.0,
            "acc": qwen_correct,
        }
    except Exception as e:
        print(f"Warning: Error in both methods verification: {e}")
